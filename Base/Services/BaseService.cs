...
namespace Base.Services
{
    public class BaseService<T> : IBaseService<T> where T : BaseEntity
    {
        ...
        public async Task<T> CreateAsync(IUnitOfWork unitOfWork, T obj)
        {
            _accessService.ThrowIfAccessDenied(unitOfWork, Enums.AccessModifier.Create, typeof(T));

            InitSortOrder(unitOfWork, obj);
            var ret = await unitOfWork.GetRepository<T>().CreateAsync(obj);
        ...
        public async Task<T> Update(IUnitOfWork unitOfWork, T obj)
        {
